...
export default function Signin() {
  ...
  function handleSubmit(e) {
    ...
    const regex =
      /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
    const regex2 =
      /^(?:(?:(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]))|(?:(?=.*[a-z])(?=.*[A-Z])(?=.*[*.!@$%^&(){}[]:;<>,.?\~_+-=|\]))|(?:(?=.*[0-9])(?=.*[A-Z])(?=.*[*.!@$%^&(){}[]:;<>,.?\~_+-=|\]))|(?:(?=.*[0-9])(?=.*[a-z])(?=.*[*.!@$%^&(){}[]:;<>,.?\~_+-=|\]))).{8,32}$/gm;
    let isValid = formData.get("email").match(regex);
    let isPassword = formData.get("password").match(regex2);
    if (formData.get("email") === "" && formData.get("password") === "") {
      alert("All Fields are required");
    } else if (!isValid) {
      alert("The provided email is not valid");
    } else if (!isPassword) {
      alert(`Password is not meets our requirement 
      At least one digit
  ...
  return (
    <>
      <div className="m-auto ovhidden px-12 bg-gradient-to-r from-pink-300 via-purple-300 to-indigo-400 sm:px-0 mx-auto">
        <div className="mx-auto h-full md:w-10/12 lg:w-6/12">
          <div className="m-auto py-12 sm:p-20 xl:w-10/12">
            ...
            <div className="mt-6 rounded-3xl border bg-gray-50 dark:border-gray-700 dark:bg-gray-800 -mx-6 sm:-mx-10 p-8 sm:p-10">
